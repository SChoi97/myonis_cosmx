...
# Ensure both the repo root and the broader tools root are importable.
...
try:
    from scripts.cosmx.utils.cosmx_visualisation_utils import (
        ...
        visualise_labels,
        _align_classification_metadata,
        _build_classification_overlay,
        _make_classification_color_map,
    )
except ModuleNotFoundError:
    from generate_h5ad.utils.cosmx_visualisation_utils import (
        create_fiji_lut,
        numericalSort,
        unpack_object_contours,
        visualise_labels,
...
def process_fov(fov_num: int, args) -> None:
    ...
    if not nuclei_adata_path.exists() or not myotube_adata_path.exists():
        print(
            f"[WARN] F{fov_str}: Missing anndata files. "
            f"nuclei={nuclei_adata_path.exists()}, myotube={myotube_adata_path.exists()} -> skipping"
        )
        return
    ...
    if len(dapi_image_path_list) < expected_patches or len(myhc_image_path_list) < expected_patches:
        print(
            f"[WARN] F{fov_str}: Not enough patches "
            f"(DAPI={len(dapi_image_path_list)}, MyHC={len(myhc_image_path_list)} "
            f"expected={expected_patches}) -> skipping"
        )
        return
    ...
    if args.classification_metadata is not None:
        classification_labels, matched_labels = _align_classification_metadata(
            nuclei_adata.obs.copy(),
            Path(args.classification_metadata),
        )
        nuclei_adata.obs["_classification_label"] = np.asarray(classification_labels, dtype=object)
        class_color_map = _make_classification_color_map(classification_labels)
        print(
            f"[INFO] F{fov_str}: matched classification metadata for "
            f"{matched_labels}/{nuclei_adata.n_obs} nuclei"
        )

    nuclei_contours_dict = nuclei_adata.uns["Object Contours"]
    myotube_contours_dict = myotube_adata.uns["Object Contours"]
    nuclei_contours, _ = unpack_object_contours(nuclei_contours_dict)
    myotube_contours, _ = unpack_object_contours(myotube_contours_dict)
    ...
    if mode == "full_fov":
        ...
        classification_overlay_full = None
        if class_color_map is not None:
            classification_overlay_full = np.zeros((full_height, full_width, 3), dtype=np.uint8)
        ...
        for target_patch in range(expected_patches):
            ...
            try:
                classification_label_list = nuclei_patch_adata.obs["_classification_label"].tolist()
            except KeyError:
            ...
            if classification_label_list is not None and class_color_map is not None:
                classification_overlay_patch = _build_classification_overlay(
                    image_shape=dapi_image.shape[:2],
                    nuclei_patch_contours=nuclei_patch_contours,
                    myotube_patch_contours=myotube_patch_contours,
                    class_labels=classification_label_list,
                    class_color_map=class_color_map,
                    dapi_lw=dapi_lw,
                    myhc_lw=myhc_lw,
                )
            else:
                classification_overlay_patch = None
            ...
            if classification_overlay_full is not None and classification_overlay_patch is not None:
                classification_overlay_full[y_start:y_end, x_start:x_end] = classification_overlay_patch

        overlays = [
            nuclei_overlay_full,
            myotube_overlay_full,
            combined_overlay_full,
            assignment_overlay_full,
        ]
        if classification_overlay_full is not None:
            overlays.append(classification_overlay_full)

        fig2, ax2 = plt.subplots(1, len(overlays), figsize=(10 * len(overlays), 10))
        plt.subplots_adjust(wspace=0.02, hspace=0)
        for axis, img in zip(np.atleast_1d(ax2), overlays):
            axis.imshow(img)
            axis.axis("off")
        ...
        if getattr(args, "save_pngs_individually", False):
            ...
            if classification_overlay_full is not None:
                imageio.imwrite(images_dir / "classification_overlay.png", classification_overlay_full)
        ...
        for target_patch in range(expected_patches):
            ...
            if getattr(args, "save_pngs_individually", False):
                merged_rgb_u8 = (np.clip(merged_rgb, 0, 1) * 255).astype(np.uint8)
                imageio.imwrite(
                    images_dir / f"F{fov_str}_P{target_patch:0{patch_index_width}d}_dapi_fluo.png",
                    dapi_patch_mod.astype(np.uint16),
                )
                imageio.imwrite(
                    images_dir / f"F{fov_str}_P{target_patch:0{patch_index_width}d}_myhc_fluo.png",
                    myhc_patch_mod.astype(np.uint16),
                )
                imageio.imwrite(
                    images_dir / f"F{fov_str}_P{target_patch:0{patch_index_width}d}_merged_fluo.png",
                    merged_rgb_u8,
                )
            ...
            overlays = [
                nuclei_overlay_patch,
                myotube_overlay_patch,
                combined_overlay_patch,
                assignment_overlay_patch,
            ]
            if classification_overlay_patch is not None:
                overlays.append(classification_overlay_patch)

            fig_a, ax_a = plt.subplots(1, len(overlays), figsize=(7 * len(overlays), 7))
            plt.subplots_adjust(wspace=0.02, hspace=0)
            for axis, img in zip(np.atleast_1d(ax_a), overlays):
                axis.imshow(img)
                axis.axis("off")
            plt.tight_layout()
            ...
            if getattr(args, "save_pngs_individually", False):
                imageio.imwrite(
                    images_dir / f"F{fov_str}_P{target_patch:0{patch_index_width}d}_dapi_overlay.png",
                    nuclei_overlay_patch,
                )
                imageio.imwrite(
                    images_dir / f"F{fov_str}_P{target_patch:0{patch_index_width}d}_myhc_overlay.png",
                    myotube_overlay_patch,
                )
                imageio.imwrite(
                    images_dir / f"F{fov_str}_P{target_patch:0{patch_index_width}d}_combined_overlay.png",
                    combined_overlay_patch,
                )
                imageio.imwrite(
                    images_dir / f"F{fov_str}_P{target_patch:0{patch_index_width}d}_instance_overlay.png",
                    assignment_overlay_patch,
                )
                if classification_overlay_patch is not None:
                    imageio.imwrite(
                        images_dir / f"F{fov_str}_P{target_patch:0{patch_index_width}d}_classification_overlay.png",
                        classification_overlay_patch,
                    )
...
def parse_args():
    ...
    parser.add_argument(
        ...
        required=True,
        type=Path,
        help="Directory containing F{FOV}_nuclei.h5ad and F{FOV}_myotubes.h5ad",
    )
    ...
    parser.add_argument(
        ...
        required=True,
        type=Path,
        help="Output directory base; per-FOV subdirs will be created",
    )
    parser.add_argument(
        "--fov-range",
        required=True,
        ...
        type=int,
        metavar=("START", "END"),
        help="Process FOVs in [START, END) with zero-padded IDs",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        metavar=("LOW", "HIGH"),
        help="Clip range for DAPI channel (default: 20 220)",
    )
    parser.add_argument(
        "--myhc-clip",
        default=(1000, 15000),
        ...
        type=int,
        metavar=("LOW", "HIGH"),
        help="Clip range for MyHC channel (default: 1000 15000)",
    )
    ...
    parser.add_argument(
        "--visualisation_mode",
        choices=["full_fov", "patch"],
        default="full_fov",
        help="full_fov (stitched) or patch (per-patch outputs)",
    )
    ...
    parser.add_argument(
        ...
        type=Path,
        help=(
            "Optional slide-specific classifier metadata CSV. When provided, "
            "an extra Voronoi classification panel is added to annotation outputs."
        ),
    )
    parser.add_argument(
        "--save_pngs_individually",
        action="store_true",
        help="If set, save each panel as an individual PNG under images/, and multi-panel figures under plots/",
    )
...
if __name__ == "__main__":
    args = parse_args()
    if args.classification_metadata is not None and not args.classification_metadata.exists():
        raise FileNotFoundError(f"--classification_metadata does not exist: {args.classification_metadata}")
