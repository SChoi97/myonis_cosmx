...
#   key=value
# Usage example:
# Rscript run_deg_condition_myotubes.R \
...
if (is.null(adata)) stop("No myotube object found in INPUT_PATH.")
...
if (ncol(adata) == 0) {
  stop("No myotubes matched the requested control/mutant cell line sets.")
}

t0_all <- Sys.time()
message(sprintf("NB-GLM DE (myotubes; control vs mutant) starting at %s",
...
message(sprintf("Selected %d myotubes from requested cell lines before additional filters", ncol(adata)))
...
if (!is.null(area_threshold)) {
  ...
  message(sprintf("%d myotubes after area filter (%s >= %.3f)", ncol(adata), area_col, area_threshold))
  if (ncol(adata) == 0) {
    stop("No myotubes pass area_threshold.")
  }
}
...
if (sum(mask_mut) == 0 || sum(mask_ctrl) == 0) {
  stop("One condition has zero myotubes after filtering.")
}
...
if (isTRUE(pseudobulk)) {
  ...
  message("pseudobulk=TRUE: aggregating myotubes by cell line; fitting without slide covariate.")
...
if (length(nonzero_totals) == 0) {
  stop("All myotubes have zero counts.")
}
...
message(sprintf("NB-GLM DE (myotubes; control vs mutant) finished in %.1f minutes",
                as.numeric(difftime(Sys.time(), t0_all, units = "mins"))))

# ---------------- SAVE OUTPUTS ----------------
out_suffix <- if (isTRUE(pseudobulk)) "_pseudobulk" else ""
out_rds <- file.path(OUTPUT_DIR, paste0("myotube_condition_deg_results", out_suffix, ".rds"))
out_rdata <- file.path(OUTPUT_DIR, paste0("myotube_condition_deg_results", out_suffix, ".RData"))
out_csv <- file.path(OUTPUT_DIR, paste0("myotube_condition_deg_results", out_suffix, ".csv"))
